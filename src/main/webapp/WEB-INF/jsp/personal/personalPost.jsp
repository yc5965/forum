...
<script type="text/html" id="PostItemTpl">
    <ul class="layui-tab-title" id="LAY_mine">
        <li data-type="mine-jie" lay-id="index" class="layui-this">我发的帖（<span>{{d.count}}</span>）</li>
    </ul>
    <div class="layui-tab-content" style="padding: 20px 0;">
        <div class="layui-tab-item layui-show">
            <ul class="mine-view jie-row">
                ...
                <li>
                    <div style="width: 70%">
                        <a class="jie-title" href="${ctx}/home/detail?id={{item.id}}" target="_blank">{{item.title}}</a>
                        <i>{{layui.laytpl.toDateString(item.createDate, 'yyyy-MM-dd HH:mm')}}</i>
                        <a class="mine-edit" href="${ctx}/home/add?id={{item.id}}">编辑</a>
                    </div>
                    <em>{{item.viewCount == null? 0:item.viewCount}}阅/{{item.commentCount == null?
                        0:item.commentCount}}答/{{item.likeCount == null? 0:item.likeCount}}赞</em>
                </li>
                {{# });}}
            </ul>
        </div>
...
<script>
    ...
    layui.use(['table', 'layer', 'laytpl', 'jquery', 'laypage'], function () {
        ...
        $.ajax({
            url: "${ctx}/bs/bsPersonal/getPersonalPost",
            type: 'post',
            success: function (r) {
                $("#rightInfo").html("");
                if (r.data.count > 0) {
                    laytpl(PostItemTpl.innerHTML).render(r.data, function (html) {
